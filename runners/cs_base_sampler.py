...
class CSBaseSampler():
    ...
    def train(self):
        ...
        dataloader = DataLoader(
            dataset, batch_size=self.config.training.batch_size, shuffle=True, num_workers=4)
        ...
        for epoch in range(self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                if step % 100 == 0:
                    ...
                    test_X = test_X / 256. * 255. + \
                        torch.rand_like(test_X) / 256.
                    if self.config.data.logit_transform:
                        test_X = self.logit_transform(test_X)

                    with torch.no_grad():
                        test_dsm_loss = dsm_score_estimation(
                            score, test_X, sigma=0.01)
    ...
    def Langevin_dynamics(self, x_mod, scorenet, n_steps=1000, step_lr=0.00002):
        ...
        with torch.no_grad():
            for _ in range(n_steps):
                ...
                print("modulus of grad components: mean {}, max {}".format(
    ...
    def test(self):
        states = torch.load(os.path.join(
            self.args.log, 'checkpoint.pth'), map_location=self.config.device)
        score = RefineNetDilated(self.config).to(self.config.device)
        score = torch.nn.DataParallel(score)

        score.module.load_state_dict(states[0])
